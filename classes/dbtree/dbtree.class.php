...
class dbtree {
    ...
    * @return object
    */
    function dbtree($table, &$db) {
        $this->db = &$db;
        $this->table = $table;
        $this->table_id    = 'cat_id';
        $this->table_left  =  'cat_left';
        $this->table_right = 'cat_right';
        $this->table_level = 'cat_level';
        unset($prefix, $table);
    }
    ...
    */
    function Clear($data = array()) {
        ...
        if (FALSE === $res) {
            ...
            $this->ERRORS_MES[] = 'internal_error';
            return FALSE;
        }
        ...
        $fld_names .= 'parent_id,'.$this->table_left . ', ' . $this->table_right . ', ' . $this->table_level;
        $fld_values .= '0,1, 2, 0';
    ...
    */
    function Insert($section_id, $condition = '', $data = array()) {
       ...
       $sql = 'UPDATE ' . $this->table . ' SET '
        ...
        $this->db->CompleteTrans(); return $this->db->Insert_ID();
    ...
    */
    ...
    /**
     Moving category 
    */
    function Move($id, $position = 'after', $condition = '') {
        $node_info = $this->GetNodeInfo($id);
        if(FALSE === $node_info) {
            $this->ERRORS_MES[] =  'cant_change_position';
            return FALSE;
        }
        list($leftId1, $rightId1, $level1) = $node_info;
        ...
        if('before' == $position) {
             $sql ="select ".$this->table_left.",".$this->table_right.",
                  ".$this->table_level." from ".$this->table." 
                  where ".$this->table_left."<'{$leftId1}' and ".$this->table_level."='{$level1}' 
                  {$condition} order by ".$this->table_left." desc limit 1";
          }
        else{
             $sql ="select ".$this->table_left.",".$this->table_right.",
                 ".$this->table_level." from ".$this->table." 
                 where ".$this->table_left.">'$leftId1' and ".$this->table_level."='{$level1}' 
                 {$condition} order by ".$this->table_left." limit 1";
           } 
        ...
        $data =$res->FetchRow();
        list($leftId2, $rightId2, $level2)   = array($data[$this->table_left], $data[$this->table_right], $data[$this->table_level]);   
      ...
      $sql = 'SELECT * FROM ' . $this->table
             . ' WHERE ' . $this->table_left . ' < ' . $leftId1
             . ' AND ' . $this->table_right . ' > ' . $rightId1
             . ' AND ' . $this->table_left . ' < ' . $leftId2
             . ' AND ' . $this->table_right . ' > ' . $rightId2                
             . ' AND ' . $this->table_level . ' = ' . ($level1-1)
             .' ORDER BY ' . $this->table_left; 
    ...
    */
    function DeleteAll($ID, $condition = '') {
        ...
        $sql = 'DELETE FROM ' . $this->table . ' WHERE ' . $this->table_left . ' BETWEEN ' . $leftId . ' AND ' . $rightId.$condition;
        $this->db->StartTrans();
        $res = $this->db->Execute($sql);
        if (FALSE === $res) {
            $this->ERRORS[] = array(2, 'SQL query error.', __FILE__ . '::' . __CLASS__ . '::' . __FUNCTION__ . '::' . __LINE__, 'SQL QUERY: ' . $sql, 'SQL ERROR: ' . $this->db->ErrorMsg());
