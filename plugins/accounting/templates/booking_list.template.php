...
<div style="background:#FFF; border:solid #3A82CC 1px; width:100%;">
    ...
    <form action="" method="get" id="filter_form">
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="3" class="tdrow1 text1">
            ...
            <tr>    <td align="right">
                    <label for="room_id"><?= $TEXT['booking_list']['filter_modal']['room'] ?></label>
                </td>
                <td>
                    <select id="room_id" name="room_id">
                        <option value="" <? if(!isset($_GET['room_id'])){echo "selected";}?>>
                            <?=$TEXT['booking_list']['filter_modal']['method']['all']?>
                        </option>
                        <? foreach($TMPL_rooms AS $k){
                            if($method_value=='all')continue;?>
                            <option value="<?=$k['ind']?>" <? if(isset($_GET['room_id'])&&$_GET['room_id']==$k['ind']){echo "selected";}?>>
                                <?=$k['name']?>
                            </option>
                        <?}?>
                    </select>
            ...
            <tr>
              <td align="right">
                    <label for="room_id">ოთახის ტიპები</label>
                </td>
                <td>
                    <select id="room__type_id" name="room__type_id">
                        <option value="" <? if(!isset($_GET['room_id'])){echo "selected";}?>>
                            <?=$TEXT['booking_list']['filter_modal']['method']['all']?>
                        </option>
                        <? foreach($TMPL_room_types AS $k){
                            if($method_value=='all')continue;?>
                            <option value="<?=$k['id']?>" <? if(isset($_GET['room__type_id'])&&$_GET['room__type_id']==$k['id']){echo "selected";}?>>
                                <?=$k['title']?>
                            </option>
                        <?}?>
                    </select>
...
<br>
<form  action="index.php?m=booking_management&tab=booking_list&action=get_invoice&invoice_type=multiple" target="_blank" method="post" id="send-multiple-invoices" >
    ...
    <table border="0" style="width:100%;" cellpadding="0" cellspacing="0" class="table-table">
        <tr>
            ...
            <td class="table-th" valign="top" nowrap>
                <?=$TEXT['booking_list']['paid']?>
            </td>
            <td class="table-th" valign="top" style="width: 94px;" align="center">
                <a  id="select-all-invoices" title="<?=$TEXT['booking_list']['select_all']?>" style="cursor:pointer; text-decoration:none;"><?=$TEXT['booking_list']['invoice']?></a><br>
        ...
        <tr class="rsf">
            ...
            <td class="table-td" align="center">
                <a href="#" class="basic"><?=$TMPL_booking[$i]['id'] ?></a>
            </td>
            <td class="table-td " style="padding-left:8px;">
                <a href="#" title="<?=$TMPL_booking[$i]['guest_id_number']?>" class="guests-all-bookings basic" guest_id_number="<?=$TMPL_booking[$i]['guest_id_number']?>" ><?=$TMPL_booking[$i]['first_name']?> <?=$TMPL_booking[$i]['last_name']?></a>
            </td>
            <td class="table-td basic" style="padding-left:8px;">
                <a href="#"  class="basic"><?=$TMPL_guests[$TMPL_booking[$i]['affiliate_id']]['first_name']?> <?=$TMPL_guests[$TMPL_booking[$i]['affiliate_id']]['last_name']?></a>
            </td>
            <td class="table-td basic" style="padding-left:8px;">
                <a href="#"  class="booking_by_room_id basic" data-id="<?=$TMPL_rooms[$TMPL_booking[$i]['room_id']]['ind']?>"><?=$TMPL_rooms[$TMPL_booking[$i]['room_id']]['name']?></a>
            </td>
            <td class="table-td" style="padding-left:8px;" nowrap>
                <a href="#" title="" class="bookings-by-check-in basic" date="<?=date('Y-m-d',strtotime($TMPL_booking[$i]['check_in']))?>" >
                    <?=date('Y-m-d',strtotime($TMPL_booking[$i]['check_in']))?>
                </a>
            </td>
            <td class="table-td" style="padding-left:8px;" nowrap>
                <a href="#" title="" class="bookings-by-check-out basic" date="<?=date('Y-m-d',strtotime($TMPL_booking[$i]['check_out']))?>" >
                    <?=date('Y-m-d',strtotime($TMPL_booking[$i]['check_out']))?>
                </a>
            </td>
            <td class="table-td" style="padding-left:8px;" nowrap>
                <?=$TMPL_booking[$i]['days_in']?>
            </td>
            <td class="table-td" style="padding-left:8px;" nowrap>
                            <a href="#" title="" class="bookings-by-method basic" method="<?=$TMPL_booking[$i]['method']?>" >
                                <?=$TEXT['booking_list']['filter_modal']['method'][$TMPL_booking[$i]['method']]?>
                            </a>
                        </td>
            <td class="table-td" style="padding-left:8px;" nowrap>
                <a href="#" title="" class="bookings-by-status basic" status_id="<?=$TMPL_booking[$i]['status_id']?>" >
                    <?=$TMPL_booking_statuses[$TMPL_booking[$i]['status_id']]['title']?>
                </a>
            </td>
            <td class="table-td" style="padding-left:8px;" nowrap>
                <a href="#" title="" class="bookings-by-food basic" food_id="<?=$TMPL_booking[$i]['food_id']?>" >
                    <?=$TMPL_food[$TMPL_booking[$i]['food_id']]['title']?>
                </a>
            </td>
            <td class="table-td" style="padding-left:8px;" nowrap>
            <?
            $debt=(int)($TMPL_booking[$i]['accommodation_price']*100);
            $debt+=(int)($TMPL_booking[$i]['services_price']*100);
            $debt-=(int)($TMPL_booking[$i]['paid_amount']*100);
            $debt-=(int)($TMPL_booking[$i]['services_paid_amount']*100);

            $debt=$debt/100;
            $out+=$debt;
            ?>
                    <a href="#"  class="basic <?=($debt>0)?'color_red':''?>"><?=$debt?></a>
            </td>
            <td class="table-td" style="padding-left:8px;" nowrap>
                <?
                $debt=(int)($TMPL_booking[$i]['paid_amount']);
                $in+=$debt;
                ?>
                <a href="#"  class="basic"><?=$debt?></a>
            </td>
            <td class="table-td" style="padding-right:8px;" nowrap align="center">
            <?
            if($TMPL_invoice_details[$TMPL_booking[$i]['id']]['a']==1){
                $a='checked';
            }else{
                $a='';
            }
            if($TMPL_invoice_details[$TMPL_booking[$i]['id']]['s']==1){
                $s='checked';
            }else{
                $s='';
            }
            ?>
                <input class="a-invoice" type="checkbox" name="invoices[<?=$TMPL_booking[$i]['id']?>][a]" title="<?=$TEXT['booking_list']['accommodation_invoice']?>" <?=$a?>> <input class="s-invoice" type="checkbox" name="invoices[<?=$TMPL_booking[$i]['id']?>][s]" title="<?=$TEXT['booking_list']['services_invoice']?>" <?=$s?>>
            </td>
            <td class="table-td" align="center" valign="middle" nowrap>
                <a href="<?=$_SERVER['PHP_SELF']?>?m=booking_management&tab=booking_list&action=view&booking_id=<?=$TMPL_booking[$i]['id']?>">
    ...
    <?}?>
        <tr>
            ...
            <td class="table-td" align="right">
                 <input class="formButton2" type="submit" style="cursor: pointer" value="<?=$TEXT['booking_list']['send']?>">
            </td>
        </tr>
    </table>
</form>
...
$(document).ready(function () {
    var tr_obj=$('.rsf');
    $.each(tr_obj,function(index,item){
        var find_debt=$(item).find('.color_red');
        var date=$(item).find('.bookings-by-check-out').attr('date');
        if(find_debt[0] && date<='<?=date('Y-m-d')?>'){
            $(item).find('.bookings-by-check-out').addClass('color_red');
        }
    })
    ...
    $(".booking_by_room_id").click(function () {
