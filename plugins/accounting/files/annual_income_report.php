...
$reports=getAnnualIncomeReport($selected_year);
if ($_POST['action'] == 'get_excel') {
    ...
    $sheet->setTitle('Annual Income Report');
    ...
    $sheet->setCellValue("A5", $TEXT['annual_income_report']['month']);
    ...
    foreach ($reports[$selected_year] AS $m=>$report) {
        $sheet->setCellValueByColumnAndRow(0, $counter +2, $report['month']);
...
$TMPL->ParseIntoVar($_CENTER, "annual_income_report");
