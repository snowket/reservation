...
for ($i=0;$i<count($data);$i++) {
    ...
    if ($data[$i]['type']=='file') {
        $out .= '<tr>';
        $out .= '<td valign="middle" class="tdrow2" nowrap width="45%" style="vertical-align: middle"><span class="text1">'.$data[$i]['title'].':</span></td>';
        $out .= '<td valign="middle" align="center" class="tdrow3" style="vertical-align: middle"><div><img '.$data[$i]["input_param"].' src="../uploads/'.$data[$i]['value'].'"></div><input style="width:220px" class="formField1" type="file" name="'.$data[$i]['input_name'].'" '.$data[$i]['input_param'].' value="'.$data[$i]['value'].'">&nbsp;<span style="color:red;">*</span>';
        $out .= '</td>';
        $out .= '</tr>';
    }
