...
<?if ($TMPL_data){?>
<h2 style="padding-top:50px; margin-bottom:20px; border-bottom:1px solid #cecece;"><?=$TEXT['price_plan']?></h2>
<?if ($TMPL_errors) {?>
...
<table width="100%" border="1" cellspacing="5" cellpadding="10" class="" style="margin-top:20px; background:#ddd; color:#000;">
	...
	?>
		...
		<tr>
			...
			<?foreach ($weekDays as $k => $v) {?>
				<td><input type="text" name="day[<?=$value['id']?>][<?=$v?>]" style="width:80px;" /><?=$TEXT['cur']?></td>
			<?}?>
		</tr>
	<?}?>
	<tr>
...
<h2 style="padding-top:50px; margin-bottom:20px; border-bottom:1px solid #cecece;">Price List</h2>
	...
	?>
		...
		<?$r=count($rooms);?>
		<form action="" method="post">
		<div style="width:120px; float:left; margin:10px 0px;">
			<table border="0" width="100%" cellpadding="0" cellspacing="0">
				...
				<?foreach ($rooms as $k4 => $v4) {?>
					<tr>
						...
						<td style="height:20px;"><?=$TEXT['cur']?></td>
					</tr>
					<tr>
		...
		<div style="width:900px; float:left; overflow-x:auto; margin:10px 0px;">
			<table border="0" width="100%" cellpadding="0" cellspacing="0">
				<tr>
