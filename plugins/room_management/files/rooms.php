<?
//*********************************************************************//
//*** Adding New rooms / Editing rooms general info ***************//
error_reporting(0);
...
//*********************************************************************//
//*** Printing list of added roomss in selected category ************//
...
//*********************************************************************//
//*** Default output - Form for adding new rooms  *******************//
else {
	drawAddForm($data);
}
...
function drawAddForm($data=array(), $errors=""){
	global $TMPL,$FIELD,$_CONF,$_CENTER;
	
	$TMPL->addVar('TMPL_errors',$errors);
	$TMPL->addVar('TMPL_lang',$_CONF['langs_all']);
	$TMPL->addVar('TMPL_title',$data['title']);
	$TMPL->addVar('TMPL_intro',$data['intro']);
	$TMPL->addVar('TMPL_item',$data);
	$TMPL->addVar('TMPL_catbox',$FIELD);
	$TMPL->addVar('TMPL_es',GetServices('extra'));
	$TMPL->addVar('TMPL_ds',GetServices('default'));
	...
	$TMPL->addVar('TMPL_blocks',GetBlocks());
	$TMPL->addVar('TMPL_types',GetRoomTypes());
	$TMPL->ParseIntoVar($_CENTER,'add_rooms_first');
}
