...
$backupUrl = $_CONF['path']['url'].'/pcms/backup/';


$errors = '';

if (!is_dir($backupDir))	$errors .= 'Backup Directory ('.$backupDir.') does not exist<br />';
else
	if (!is_writable($backupDir)){
		$errors .= 'Backup Directory ('.$backupDir.') is not writable - chmod to 0777<br />';
	}

if (!empty($errors))	$TMPL->addVar("msg",$errors);
else					$backupEnabled = true;
...
function DeleteBackup($filename){
	global $DB, $backupDir, $FUNC, $SELF, $TMPL;
...
function DisplayBackups(){
  global $DB, $backupDir, $backupUrl, $backupEnabled, $SELF;

  clearstatcache();

  $out = '<table width="100%" border="0" cellpadding="5" cellspacing="0">
        <tr>
          <td class="tdrow1" colspan="6">Backup Directory: "'.$backupDir.'"</td>
        </tr>
        <tr>
          <td class="tdrow1" width="50%">File Name</td>
          <td class="tdrow1">Size</td>
          <td class="tdrow1">Last Modified</td>
      ...
      for($i = 0;$i < count($filestats); $i++){
        if(empty($filestats[$i]['error'])){
          $out .= '<tr>
              ...
              <td class="tdrow3">' . DisplayDate($filestats[$i]['mtime']) . '</td>
              <td class="tdrow3"><a href="'.$SELF.'&dbaction=restorebackup&filename=' . $filestats[$i]['file'].
              ...
              <td class="tdrow3"><a href="' . $backupUrl . $filestats[$i]['file']. '">Download</a></td>
              <td class="tdrow3"><a href="'.$SELF.'&dbaction=deletebackup&filename=' . $filestats[$i]['file'].
              '" onclick="return confirm(\'Are you sure you wish to delete this backup file?\');">Delete</a></td>
            </tr>';
        }
        else{
          $out .= '<tr>
...
function DisplayTables(){
	...
	$out .= '<script language="JavaScript" src="../js/functions.js" type="text/javascript"></script>';
	$out .= '<form method="post" action="" name="tables" id="pcms_tables">
			<input type="hidden" name="dbaction" value=""/>';

	$out .='<table border="1" width="100%" cellpadding="5" cellspacing="0">
	      <tr>
	        <td class="tdrow1"><input type="checkbox" checkall="group" onclick="javascript: return CheckAll(\'pcms_tables\',\'tablenames\');" style="cursor:pointer;"></td>
            <td class="tdrow1">Table Name</td>
            <td class="tdrow1">Rows</td>
            <td class="tdrow1">Data Length</td>
            <td class="tdrow1">Index Length</td>
            <td class="tdrow1">Overhead</td>
            <td class="tdrow1" colspan="4">&nbsp;</td>
	      </tr>';
	/*$gettables	= array();
	...
	$out .= '<tr>
        <td class="tdrow1" colspan="6">&nbsp;</td>
                ...
                <td class="tdrow1"><input type="submit" value="Backup" onclick="document.forms[\'tables\'].dbaction.value = \'backupall\';" ' . 
...
if(!empty($action)){
  switch ($action){
    ...
    case 'backuptable':
      ...
      break;
    case 'backupall':
      PrintResults('Backup Table Results', BatchBackupTable($tablenames));
      break;
    case 'restorebackup':
