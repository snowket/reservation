...
(isset($_GET['in_end_date']) && isset($_GET['in_start_date']))? $order_clause.="ORDER BY B.check_in ASC" : $order_clause.="ORDER BY B.id DESC";
...
}elseif ($_POST['action_dbl'] == 'delete_dbl_booking') {
    $booking_id = (int)$GET['booking_id'];
    delete_dbl_booking($booking_id);
    header("Location:" . $SELF);
}
elseif ($_POST['action_dbl'] == 'cancel_dbl_booking') {
    $booking_id = (int)$GET['booking_id'];
    cancel_dbl_booking($booking_id);
    header("Location:" . $SELF);
}
 elseif ($_POST['action'] == 'update_booking_comment') {
    ...
    header("Location: index.php?m=booking_management&tab=booking_dbl_list&action=view&booking_id=" . (int)$GET['booking_id']);
    ...
    if ($_SESSION['pcms_user_group'] <=2) {
        ...
        header("Location: index.php?m=booking_management&tab=booking_dbl_list");
    } else {
    ...
    changeCheckinCheckout((int)$_GET['booking_id'], $_POST['new_checkin'], $_POST['new_checkout']);
...
if ($_GET['action'] == "view") {
    if (isset($_GET['booking_id'])) {
            ...
            $booking_master=getDBLbooking_co($booking_id);
            $guest=getGuestByID($booking_master['booking_user_id']);
           # dd($booking_master);
...
//    }
    ...
    }elseif($_GET['invoice_type']=='accommodation_dbl' || $_GET['invoice_type'] == 'services_dbl' || $_GET['invoice_type'] == 'full_dbl' ){
        if (isset($_GET['booking_id'])) {
            $lang=(isset($_GET['lang'])?$_GET['lang']:LANG);
            $booking_id = (int)$_GET['booking_id'];
            $query = "SELECT * FROM {$_CONF['db']['prefix']}_booking WHERE dbl_res_id={$booking_id}";
            ...
            foreach($bookings as &$book){
              if($book['responsive_guest_id']==0){
                ...
                if ($_GET['invoice_type'] == 'accommodation_dbl') {
                    ...
                    $invoice_identifier .= $book['id'] . ":s|";
                } else {
                    $invoice_identifier .= $book['id'] . ":a|" . $book['id'] . ":s|";
                }
                $res_guest[$book['id']] = getGuestByID($book['responsive_guest_id']);
                $fg=$book['foreign_guest_ids'];
                $fg=explode('|',$fg);
                $fg=array_slice($fg, 1, -1);
                ...
                foreach ($booking_days[$book['id']] AS $booking_day) {
                    $booking_days_ids[$book['id']][] = $booking_day['id'];
                }
                $tmp_arr = implode(", ", $booking_days_ids[$book['id']]);
                $query = "SELECT BDS.*,RS.*
                FROM {$_CONF['db']['prefix']}_booking_daily_services BDS
                LEFT JOIN {$_CONF['db']['prefix']}_room_services RS ON RS.id=BDS.service_id
                WHERE BDS.booking_daily_id IN ({$tmp_arr}) ";
                ...
                $result = $CONN->Execute($query) or $FUNC->ServerError(__FILE__, __LINE__, $CONN->ErrorMsg());
                $booking_daily_services[$book['id']] = $result->GetRows();
                ...
                $TMPL->addVar("TMPL_Rguest", $res_guest);
                $TMPL->addVar("TMPL_Fguest", $fg_guests);
                $TMPL->addVar('TMPL_food', $food_obj);
                $TMPL->addVar("TMPL_payment_methods", getAllPaymentMethods());
                $TMPL->addVar("TMPL_booking_days", $booking_days);
                $TMPL->addVar("TMPL_all_extra_services", getAllServices());
                $TMPL->addVar("TMPL_booking_daily_services", $booking_daily_services);
                ...
                $TMPL->addVar('TMPL_all_statuses', getAllStatuses());
                $TMPL->addVar("TMPL_settings", $settings);
                $guest = getGuestByID($book['guest_id']);
                $TMPL->addVar("TMPL_guest", $guest);

                if ($_GET['invoice_type'] == 'accommodation_dbl') {
                    $template = "accommodation_invoice_dbl";
                    $invoices[$book['id']]['a'] = "on";
                } elseif ($_GET['invoice_type'] == 'services_dbl') {

                    $template = "services_invoice_dbl";
                    $invoices[$book['id']]['s'] = "on";
                } elseif ($_GET['invoice_type'] == 'full_dbl') {
                    $template = "full_invoice_dbl";
                    $invoices[$book['id']]['a'] = "on";
                    $invoices[$book['id']]['s'] = "on";
                }
                $invoice_number = addInvoice($invoice_identifier);
    ...
    if (isset($_GET['booking_id']) && isset($_GET['daily_service_id'])) {
        ...
        updateBookingFinances($booking_id);
        header("Location: index.php?m=booking_management&tab=booking_dbl_list&action=view&booking_id=" . $booking_id);
    }
    ...
    $pageBar = "";
    $bookings = getDblJoinedBookings($where_clause,$order_clause);
    $lastid=0;
    $master=array();
    foreach ($bookings as $key => $value) {
    ...
    $TMPL->addVar("TMPL_settings", $SETTINGS);
    $TMPL->addVar("TMPL_booking", $master);
    ...
    $TMPL->ParseIntoVar($_CENTER, "booking_list_dbl");

}
function delete_dbl_booking($id){
    global $CONN, $FUNC, $_CONF, $ALOG;
      $select="SELECT * FROM {$_CONF['db']['prefix']}_booking where id={$id} OR dbl_res_id={$id}";
      $select = $CONN->Execute($select) or $FUNC->ServerError(__FILE__, __LINE__, $CONN->ErrorMsg());
      $select=$select->getRows();
      foreach($select as $k => $value){
        deleteBooking($value['id']);
      }
      delete_dbl_user_info($id);
}
function cancel_dbl_booking($id){
    global $CONN, $FUNC, $_CONF, $ALOG;
      $select="SELECT * FROM {$_CONF['db']['prefix']}_booking where id={$id} OR dbl_res_id={$id}";
      $select = $CONN->Execute($select) or $FUNC->ServerError(__FILE__, __LINE__, $CONN->ErrorMsg());
      $select=$select->getRows();
...
function delete_dbl_user_info($id){
    global $CONN, $FUNC, $_CONF, $ALOG;
    $query = "DELETE
    FROM {$_CONF['db']['prefix']}_booking_dbl
    WHERE booking_master_id=" . $id;
    $result = $CONN->Execute($query) or $FUNC->ServerError(__FILE__, __LINE__, $CONN->ErrorMsg());
}
function getAlldblBookings($id){
      global $CONN, $FUNC, $_CONF, $ALOG;
        $query="SELECT * FROM cms_booking WHERE dbl_res_id=".$id;
