...
$daily_query = "SELECT DISTINCT(B.id),B.adult_num,B.child_num,B.food_id,BRS.title,BD.date FROM {$BOOKING} B
    LEFT JOIN {$BOOKING_DAILY} BD on BD.booking_id=B.id
...
foreach ($daily_query as $key => $booking) {
    if (!$food_ids[$booking['food_id']]) {
        $food_ids[$booking['food_id']] = $r;
        $r += 2;
    }
...
//        $sheet->setCellValueByColumnAndRow(4, $i + 2, $check_out);
        $k = 0;
        foreach ($items[$date[$i]]['food'] as $kr => $food) {
            $sheet->setCellValueByColumnAndRow(4 + $food_ids[$kr], $i + 9, $food['title']);
            $sheet->setCellValueByColumnAndRow(5 + $food_ids[$kr], $i + 9, $food['count']);
            $k += 2;
        }
