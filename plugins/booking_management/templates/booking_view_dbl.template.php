...
<input type="hidden" name="booked_array" value="<?=implode(',',$bookings_array)?>">
<table class="master">
	<tr>
		<td >
					...
					<td>
						<table>
							<tr>
								...
								<td>
									<input type="text" id="check_in" name="check_in" class="calendar-icon_x hasDatepicker" required="" value="<?=date('d-m-Y',strtotime($booking_master['booking_check_in']))?>">
								</td>
							</tr>
						</table>
					</td>
					...
					<td>
						<table>
							<tr>
								...
								<td>
									<input type="text" id="check_out" name="check_out" class="calendar-icon_x hasDatepicker" required="" value="<?=date('d-m-Y',strtotime($booking_master['booking_check_out']))?>">
								</td>
							</tr>
						</table>
					</td>
...
<table class="master_room">
	<?php foreach ($TMPL_bookings as $key => $value): ?>
		<input type="hidden" name="booking_ids[]" value="<?=$value['id']?>">
		<input type="hidden" name="booking_rooms[]" value="<?=$value['room_id']?>">
		<tr id="master_tr" class="master_id_<?=$key?> levok">
		<td style="width: 410px;" >
			<table class="informer">
				<tr>
					...
					<td>
						<select name="room_id[]" id="room_id" class="select select_room" date-idl="<?=$value['room_id']?>">
					...
					<td>
						<select name="food_id[]" id="food" class="select select_food">
										...
										<?php foreach ($TMPL_all_food as $k => $food): ?>
											<option value="<?=$food['id']?>" <?=$value['food_id']==$food['id']?'selected':''?> price="<?=$food['price']?>"><?=$food['title']?></option>
										<?php endforeach ?>
						</select>
					</td>
				</tr>
			...
			<table class="informer">
				<tr>
					...
					<td>
						<input type="number" name="person_count[]" id="adult_id" value="<?=$value['adult_num']?>">
					</td>
				</tr>
				<tr>
					<td>
					...
					<td>
						<input type="text" id="check_in_x" name="check_in_x[]" class="calendar-icon_d hasDatepicker check_in_x" value="<?=date('d-m-Y',strtotime($value['check_in']))?>">
		...
		<td>
			<table class="informer">
				<tr>
					...
					<td>
						<input type="number" name="child_count[]" id="child_id" value="<?=($value['child_num'])?$value['child_num']:'0'?>">
					...
					<td>
						<input type="text" id="check_out_x" name="check_out_x[]" class="calendar-icon_d hasDatepicker check_out_x" value="<?=date('d-m-Y',strtotime($value['check_out']))?>">
		...
		<td  style="height: 60px;">
			<input type="hidden" name="room_price[]" class="room_price" value='<?=$value['accomodation_price']?>'>
			<table class="row_former" style="height: 100%;">
				<tr>
				<td >
					<button class="delete_button_room_tmp" style="height:100%;" onclick="deleteRoomtmp(this)" data-id="<?=$value['id']?>"><i class="fa fa-times"></i></button>
				</td>
				</tr>
	...
	<tr id="button_tr">
		<td colspan="4">
			<a onclick="AddRowRoomTmp(this); " data-id="<?=$key?>"  class="addrowroomtmp">ოთახის დამატება</a>
		</td>
	</tr>
</table>
<div class="divider"></div>
<table class="master">
	<tbody>
			...
			<td colspan="2">
				<table class="informer">
					<tr>
						...
						<td>
              <input type="number" name="all_out" class="all_out" placeholder="ჯამური" value="<?=$TMPL_bookings[$key]['fixed_discount']*($key+1)?>">
            </td>
					</tr>
				</table>
			</td>
		...
		<tr>
			...
			<td colspan="1">
				<button class="book_it">დაჯავშნვა</button>
			</td>
      <td colspan="1">
				<button class="delete_it">ჯავშნის წაშლა</button>
				<button class="cancel_it">ჯავშნის გაუქმება</button>
			</td>
		</tr>
	</tbody>
</table>
    ...
    <!--GUEST MODAL END-->
    <form class="delete_dbl_booking"  method="post">
      <input type="hidden" name="m" value="<?=$_GET['m']?>">
      <input type="hidden" name="tab" value="<?=$_GET['booking_dbl_list']?>">
      <input type="hidden" name="booking_id" value="<?=$_GET['booking_id']?>">
      <input type="hidden" name="action_dbl" value="delete_dbl_booking">
    </form>
    <form class="cancel_dbl_booking"  method="post">
      <input type="hidden" name="m" value="<?=$_GET['m']?>">
      <input type="hidden" name="tab" value="<?=$_GET['booking_dbl_list']?>">
      <input type="hidden" name="booking_id" value="<?=$_GET['booking_id']?>">
      <input type="hidden" name="action_dbl" value="cancel_dbl_booking">
    </form>
<script>
  "use strict";
	var free_rooms=[];
  ...
  $('.delete_it').on('click',function(e){
    e.preventDefault();
  $('.delete_dbl_booking').submit();
  })
  $('.cancel_it').on('click',function(e){
    e.preventDefault();
  $('.cancel_dbl_booking').submit();
  })
 ...
 $( "#guest_search_field").autocomplete({
	...
	function regenPrice(){
	 ...
	 $(function() {
			...
			$.each(cx,function(index,item){
				...
				$(item).datepicker(Object.assign(dateOptions,{'Date':check_out}));

			});
			var rx=$('.check_in_x');
			$.each(rx,function(index,item){
				$(item).datepicker(Object.assign(dateOptions,{'Date':check_in}));
			});
			var $levok=$('.levok');
			$.each($levok,function(index,item){
				var checkin=$(item).find('.check_in_x').val();
				var checkout=$(item).find('.check_out_x').val();
				$.ajax({
					  type: "POST",
					  ...
					  success: function(data) {
						...
						var classList = $(item).attr('class').split(/\s+/);
						fillFreeRooms(classList[0]);
						 var ro=$('.select_room');
			             <?php foreach ($TMPL_bookings as $key => $value): ?>
			             	$(ro[<?=$key?>]).val('<?=$value['room_id']?>').change();
			             	selectedRooms.push('<?=$value['room_id']?>');
			             <?php endforeach ?>
					  }
				});
			});
	...
	$(function() {
	    ...
	    var from = $("#check_in").datepicker(dateOptions),

          to = $( "#check_out" ).datepicker(dateOptions),

	        from_x = $("#check_in_x").datepicker(dateOptions),
	        to_x = $( "#check_out_x" ).datepicker(dateOptions);
   ...
   $('select.select_room').live('change', function (e) {
     ...
     regenPrice();
      ...
      var el_id=$(element).data('id');
	    ...
	    if (strconfirm == true) {
        var request = $.ajax({
            url: "index_ajax.php?cmd=delete_dbl_booking_room",
            method: "POST",
            data: {el_id: el_id},
            dataType: "json"
        });
        ...
        request.done(function (msg) {
          var dat=jQuery.parseJSON(JSON.stringify(msg));
          if(dat.code=1){
            location.reload();
          }
          else{
            alert('Cant Delete Booking Room');
          }
        });
	...
	function fillFreeRooms(element){
		var h='<option> ოთახები</option>';
		$.each(free_rooms,function(index,value){
  ...
  function display_check_in_check_out(element){
		...
		$(xin).datepicker('reset').datepicker('destroy').datepicker(dateOptions);
    $(xin).datepicker('setDate',$('#check_in').datepicker('getDate',true));
		$(xon).datepicker('reset').datepicker('destroy').datepicker(Object.assign(dateOptions,{'Date':$('#check_out').datepicker('getDate',true)}));
	...
	function getAllDateFood(from, to){
      ...
      $('.book_it').click(function(e){
        ...
        if(error_room || error_food){
          ...
          // $('<input type="submit">').hide().appendTo('#update_booking').click().remove();
        }

      })
      <?php
      if($_SESSION['dbl_save']=='OK'){ ?>
        swal("ჯავშანი შექმნილია", "გადამისამართება ჯავშანზე ", "success",{
        timer: 3000,
      });
  <?    }elseif($_SESSION['dbl_save']=='NO'){  ?>
        swal("შეცდომა", "<?=$_SESSION['dbl_save_data'][0][0]?>", "error",{
        timer: 3000,
      });
    <? }
    unset($_SESSION['dbl_save']);
    unset($_SESSION['dbl_save_data']);
    ?>
</script>
