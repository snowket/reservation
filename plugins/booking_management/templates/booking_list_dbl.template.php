            ...
            <tr>
                ...
                <td>
                    <select id="guest_type" name="guest_type">
                        <option value="" <?php if (!isset($_GET['guest_type'])) {echo "selected";}?>><?=$TEXT['guest_type']['all'];?></option>
                        <option value="non-corporate" <?php if (isset($_GET['guest_type']) && $_GET['guest_type'] == 'non-corporate') {echo "selected";}?>><?=$TEXT['guest_type']['non-corporate'];?></option>
                        <option value="company" <?php if (isset($_GET['guest_type']) && $_GET['guest_type'] == 'company') {echo "selected";}?>><?=$TEXT['guest_type']['company'];?></option>
                        <option value="tour-company" <?php if (isset($_GET['guest_type']) && $_GET['guest_type'] == 'tour-company') {echo "selected";}?>><?=$TEXT['guest_type']['tour-company'];?></option>
                    </select>
                </td>
                <td align="right">
                ...
                <td>
                    <select id="status_id" name="status_id">
                        <option value="" <?php if (!isset($_GET['status_id'])) {echo "selected";}?>><?=$TEXT['booking_list']['filter_modal']['all'];?></option>
                        <?php foreach ($TMPL_booking_statuses as $status) {
    if ($status['id'] == 2) {
        continue;
    }
    ?>
                            <option value="<?=$status['id'];?>" <?php if (isset($_GET['status_id']) && (int) $_GET['status_id'] == $status['id']) {echo "selected";}?>>
                                <?=$status['title'];?>
                            </option>
                        <?php }?>
                    </select>
            ...
            <tr>
             ...
             <select id="method" name="method">
                 <option value="" <?php if (!isset($_GET['method'])) {echo "selected";}?>>
                    <?=$TEXT['booking_list']['filter_modal']['method']['all'];?>
                 </option>
                 <?php foreach ($TEXT['booking_list']['filter_modal']['method'] as $method_value => $method_title) {
    if ($method_value == 'all') {
        continue;
    }
    ?>
                     <option value="<?=$method_value;?>" <?php if (isset($_GET['method']) && $_GET['method'] == $method_value) {echo "selected";}?>>
                         <?=$method_title;?>
                     </option>
                 <?php }?>
             </select>
                   ...
                   <td>
                       ...
                       <select name="floor" id="floor">
                           ...
                           <?php for ($i = 1; $i <= $TMPL_max_floor['floor']; $i++) {?>
...
<form  action="index.php?m=booking_management&tab=booking_dbl_list&action=get_invoice&invoice_type=multiple" target="_blank" method="post" id="send-multiple-invoices" >
    ...
    <table border="0" style="width:100%;" cellpadding="0" cellspacing="0" class="table-table">
        ...
        <?php $i = 0;foreach ($TMPL_booking as $booking) {
    ?>
        <tr class="master_tr_class">
            ...
            <td class="table-td" valign="top" style="text-align: center;"><?=$booking['info']['price']-$booking['info']['left_amo'];?></td>
            ...
            <td class="table-td" valign="top" style="text-align: center;">
                 <a target="_blank" href="index.php?m=booking_management&tab=booking_dbl_list&action=view&booking_id=<?=$booking['info']['booking_master_id'];?>"><?=$TEXT['change_room_modal']['edit'];?></a>
            </td>
        </tr>
