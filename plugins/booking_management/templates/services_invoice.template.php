                ...
                <? for($j=0; $j<count($TMPL_booking_daily_services); $j++){
                    ...
                    if($TMPL_booking_daily_services[$j]['booking_daily_id']==$TMPL_booking_days[$i]['id']){
                        if($TMPL_booking_daily_services[$j]['service_type_id']!=2&&$TMPL_booking_daily_services[$j]['service_type_id']!=9){
                          if(!isset($tj[$TMPL_booking_daily_services[$j]['service_id']])){
                            $tj[$TMPL_booking_daily_services[$j]['service_id']]['name']  = $TMPL_booking_daily_services[$j]['service_title'];
                            $tj[$TMPL_booking_daily_services[$j]['service_id']]['price'] = $TMPL_booking_daily_services[$j]['service_price'];
                            $tj[$TMPL_booking_daily_services[$j]['service_id']]['count'] = 1;
                          }else{
                            $tj[$TMPL_booking_daily_services[$j]['service_id']]['price'] += $TMPL_booking_daily_services[$j]['service_price'];
                            $tj[$TMPL_booking_daily_services[$j]['service_id']]['count'] ++;
                          }
                    ...
                    foreach ($tj as $key => $value) { ?>
                      <tr>
                          <td nowrap><?=$value['name']?> X <?=$value['count']?></td>
                          <td nowrap align="right" width="30px" class="dslr"><?=$value['price']?></td>
                      </tr>
                  <?  } ?>
...
<table width="100%" border="1" style="width:100%; border-collapse: collapse;" cellpadding="2" cellspacing="0">
    ...
    <tr>
        ...
        <td align="right" class="dslr"><?=$TMPL_booking['services_paid_amount']?></td>
    </tr>
    <tr >
        ...
        <div id="total_amount_due" style="font-weight: bold" class="dslr"> <?=($total_price-$TMPL_booking['services_paid_amount'])?></div>
        </td>
    </tr>
</table>
<br>
