...
$backupDir=RESERVATION_ROOT.$_CONF['path']['db_backup'];
$backupUrl ="..".$_CONF['path']['db_backup'];
...
function DisplayBackups(){
    ...
    if($dir = @opendir($backupDir)){
        while (false !== ($file = readdir($dir))){
            ...
            if((substr($file,0,1)!='.') && (strpos(strtolower($file),'.sql') > 0)){
                if($stats = @stat($backupDir . $file)){
                    ...
                    $file_stats[] = array('file' => $file, 'error' => true);
                }
            }
        }
        ...
        krsort($file_stats);
        if(!empty($file_stats)){
            //@sort($file_stats,SORT_STRING);
            foreach($file_stats as $mk=>$file_stat){
                if(empty($file_stat['error'])){
                    $out .= '<tr>
                              <td class="tdrow3">' . $file_stat['file'] . '</td>
                              <td class="tdrow3">' . DisplayReadableFilesize($file_stat['size']) . '</td>
                              <td class="tdrow3">' . $file_stat['mtime'] . '</td>
                              <!--td class="tdrow3">
                              <a href="'.$SELF.'&dbaction=restorebackup&filename=' . $file_stat['file'].
                              ...
                              <td class="tdrow3">
                              <a href="'.$SELF.'&dbaction=downloadbackup&filename=' . $file_stat['file'].'">'.$TEXT['db_backup']['list_table']['download'].'</a></td>
                              <td class="tdrow3"><a href="'.$SELF.'&dbaction=deletebackup&filename=' . $file_stat['file'].
...
function DownloadBackup($file_name){
    global $backupDir;
    header_remove ();
